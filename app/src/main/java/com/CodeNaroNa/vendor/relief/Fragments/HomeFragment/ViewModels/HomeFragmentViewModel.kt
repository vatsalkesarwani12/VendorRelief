...
@Suppress("unused")
class HomeFragmentViewModel(private val applicationContext: Context, private val centralRepository: CentralRepository) : ViewModel() {
    ...
    fun getVendors(stateName: String, cityName: String, shopCategory: String): Boolean {
        ...
        viewModelScope.launch(Dispatchers.IO) {
            try {
               val list = centralRepository.getVendorData(stateName,cityName, shopCategory)
                if(list.size ==0)
