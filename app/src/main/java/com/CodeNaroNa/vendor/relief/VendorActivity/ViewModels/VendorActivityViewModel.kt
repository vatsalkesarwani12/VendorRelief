...
@Suppress("unused")
class VendorActivityViewModel(private val applicationContext: Context, private val centralRepository: CentralRepository) : ViewModel() {
    ...
    private fun getStateList() {
        viewModelScope.launch(Dispatchers.IO) {
            try {
                val stateList = centralRepository.getStateList()
    ...
    fun getCityList(stateName: String) {
        viewModelScope.launch(Dispatchers.IO) {
            try {
                ...
                val cityList = centralRepository.getCityList(stateName)
    ...
    fun getVendor(phoneNumber: String) {
        viewModelScope.launch(Dispatchers.IO) {
            try {
                val data2 = centralRepository.getVendor(phoneNumber)
    ...
    fun getPrecautionData() {
        viewModelScope.launch(Dispatchers.IO) {
            try {
                val list = centralRepository.getPrecautionData()
        ...
        viewModelScope.launch(Dispatchers.IO) {
            try {
                if (checkValidity(shopName, shopCategory, state, city, address, openTime, closeTime)) {
                    ...
                    centralRepository.updateVendor(userData)
